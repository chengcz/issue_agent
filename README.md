...
`pending`/`planned`/`failed`/`blocked`/`split`/`human_review`（`split` 是人工否决拆分、按单 Issue
重新规划的方式；`human_review` 供评审者关闭 PR 未合并后重跑——重跑会复用已有 PR 与已批准的
最终状态，不会重复评审）；运行中或已到 `done` 的任务会被拒绝。
...
2. 按 [GitHub Issue 模板](docs/issue-template.md) 填写完整任务，确保范围可由一个 PR 完成。
...
gh issue create \
  ...
  --body "请按 docs/issue-template.md 中的模板填写完整任务" \
  --label enhancement \
  --label agent:claude \
  --label agent-ready
```
...
模板与网页发布步骤见 [docs/issue-template.md](docs/issue-template.md)。
